...
export function LiveSupportWidget() {
  ...
  // Track window scrolling position for button offset and style alterations
  useEffect(() => {
    const handleScroll = () => {
    ...
    <div className="fixed bottom-6 z-50 font-sans transition-all duration-300 ease-in-out">
      ...
      {isOpen && (
        ...
        >
          ...
          {/* CRITICAL FIX: Isolated Sub-component handles all React Query calls.
            It only renders inside the client browser context once the widget opens, 
            preventing Server-Side context crash errors completely.
          */}
          <ChatWindowContent closeWidget={() => setIsOpen(false)} />
...
/* SUB-COMPONENT WRAPPER TO ISOLATE REACT QUERY CONTEXT HOOKS */
function ChatWindowContent({ closeWidget }: { closeWidget: () => void }) {
  const { user, loading } = useAuth();
  const qc = useQueryClient(); // Safely called within the active context provider tree now
  ...
  // 2. Fetch history database messages 
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-xs text-slate-400 mb-4 max-w-[240px]">
          We require authentication to log messages directly to your shipment profile.
        </p>
        <Link
          to="/login"
          onClick={closeWidget}
        ...
        <div className="space-y-4 flex flex-col">
          {messages?.map((msg) => {
            ...
            return (
              ...
              >
                <div
                  className={`px-4 py-2.5 rounded-2xl text-[14px] leading-relaxed shadow-sm ${isUser
      ...
      >
        <div className="relative flex-1 flex items-center">
          ...
          <Button
            ...
            size="icon"
            className={`absolute right-1.5 top-1/2 -translate-y-1/2 h-7 w-7 rounded-lg transition-all ${inputMessage.trim()
